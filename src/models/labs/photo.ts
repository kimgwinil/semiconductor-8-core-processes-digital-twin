...
/**
 * P3 포토리소그래피 실습 3단계 — PLN `03_실습3단계명세.md` §S5(722행)·§S6(784행)·§S7(865행) 배선.
 *
 * ═══════════════════════════════════════════════════════════════════════════
...
/* ══════════ 🔴 기초(S5) 초점 응답 차트 (PLN 명세 816) ══════════
 * 명세 816 원문: 「… 화면에는 초점–노광 매트릭스(Bossung) 미니 차트가 **아래로 볼록한 U 자**로
 *   그려지고, 현재 위치가 U 자 위에서 노란 점으로 표시된다.」 (같은 절 §4 의 마지막 항목)
 *
 * ⛔ **명세 809 의 「공중상 강도 프로파일」은 그리지 않았다 — 지어내지 않으려고 뺐다.**
 *    809 가 요구하는 값(피크 0.95 · 바닥 0.08 · 대비 0.845 → 0.638 · NILS 3.4 → 2.3 ·
 ...
 *
 * 🔴 ~~`judgesOutputs` 를 달지 않는다. 「판정은 이 차트에서 한다」는 명세 427(P1 σ_D)에만 있는~~
 *    ~~문장이고, 명세가 이 차트에 그렇게 못박은 문장은 S6(881행)의 Bossung 등고선 쪽이다.~~
 *    ~~여기서는 규격선만 긋는다 — 없는 권한을 화면에 적지 않는다.~~
 *
 * 🔴🔴 **2026-08-21 정정(개발팀장) — `judgesOutputs: ['cdNm']` 을 단다.**
 *    위 주석의 **명세 독해는 맞다**: 「판정은 이 차트에서 한다」는 정본
 *    `03_실습3단계명세.md` 전체에서 **정확히 1회**(현재 492행, wafer σ_D 확대 차트)만 나오며
 *    일반 규약이 아니다. 그러나 **결론이 틀렸다** — `judgesOutputs` 는 「명세가 권한을 주었는가」가
